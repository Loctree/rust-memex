...
mod tests {
    ...
    //
    // These tests prove that the MCP protocol surface is identical across
    // transports (stdio vs SSE/HTTP). A future change that accidentally makes
    // initialize, tools/list, or error semantics transport-dependent will
    // fail here before it ships.
    // =========================================================================

    #[test]
    fn parity_initialize_is_transport_agnostic() {
        // shared_initialize_result() is called for BOTH transports.
        // Calling it twice must yield byte-identical JSON.
        let a = shared_initialize_result();
        let b = shared_initialize_result();
        assert_eq!(a, b, "initialize result must be deterministic and transport-agnostic");

        // Structural contract: protocolVersion, serverInfo, capabilities
        assert!(a["protocolVersion"].is_string());
        assert!(a["serverInfo"]["name"].is_string());
        assert!(a["serverInfo"]["version"].is_string());
        assert!(a["capabilities"]["tools"].is_object());
        assert!(a["capabilities"]["resources"].is_object());
    }

    #[test]
    fn parity_tools_list_is_transport_agnostic() {
        let a = shared_tools_list_result();
        let b = shared_tools_list_result();
        assert_eq!(a, b, "tools/list result must be deterministic and transport-agnostic");
    }

    #[test]
    fn parity_tool_catalog_covers_all_14_tools() {
        use super::McpTool;

        let result = shared_tools_list_result();
        let tools = result["tools"].as_array().expect("tools must be array");
        assert_eq!(tools.len(), McpTool::ALL.len(), "tool count must match McpTool::ALL");

        // Every tool in ALL must appear in the catalog with matching name
        for expected in McpTool::ALL {
            let found = tools.iter().any(|t| t["name"].as_str() == Some(expected.name()));
            assert!(found, "tool '{}' missing from tools/list", expected.name());
        }
    }

    #[test]
    fn parity_every_tool_has_valid_input_schema() {
        let result = shared_tools_list_result();
    ...
    #[test]
    fn parity_health_transport_is_the_only_transport_dependent_field() {
        use super::McpTransport;

        // Document the ONLY known transport-dependent behavior:
        // McpTransport::Stdio returns None (no transport field in health)
        // McpTransport::HttpSse returns Some("mcp-over-sse")
        assert_eq!(McpTransport::Stdio.health_transport(), None);
        assert_eq!(McpTransport::HttpSse.health_transport(), Some("mcp-over-sse"));
    }

    #[test]
    fn parity_parse_errors_are_transport_independent() {
        // Parse errors happen BEFORE transport is consulted, so they're
        // guaranteed identical. Verify the structure is correct JSON-RPC 2.0.
        let err = parse_jsonrpc_payload("not json", 1024).unwrap_err();
        assert_eq!(err["jsonrpc"], "2.0");
        assert_eq!(err["error"]["code"], -32700);
        assert!(err.get("id").is_none(), "parse error must not have id field");

        let err = parse_jsonrpc_payload(&"x".repeat(2000), 1024).unwrap_err();
        assert_eq!(err["error"]["code"], -32600);
    }

    #[test]
    fn parity_method_routing_covers_all_mcp_methods() {
        use super::McpMethod;

        // Verify the method router recognizes all standard MCP methods
        assert_eq!(McpMethod::from_name("initialize"), Some(McpMethod::Initialize));
        assert_eq!(McpMethod::from_name("tools/list"), Some(McpMethod::ToolsList));
        assert_eq!(McpMethod::from_name("tools/call"), Some(McpMethod::ToolsCall));

        // Unknown methods return None (will produce -32601 error)
        assert_eq!(McpMethod::from_name("resources/list"), None);
        assert_eq!(McpMethod::from_name(""), None);
        assert_eq!(McpMethod::from_name("INITIALIZE"), None);
    }

    #[test]
    fn parity_tool_routing_roundtrips_all_names() {
        use super::McpTool;

        // Every tool name must survive from_name → name roundtrip
        for tool in McpTool::ALL {
            let name = tool.name();
            let parsed = McpTool::from_name(name);
            assert_eq!(
                parsed,
                Some(tool),
                "tool '{}' must roundtrip through from_name",
                name
            );
        }

        // Unknown tool names return None
        assert_eq!(McpTool::from_name("nonexistent"), None);
        assert_eq!(McpTool::from_name(""), None);
    }

    #[test]
    fn parity_jsonrpc_framing_is_transport_agnostic() {
        // jsonrpc_success and jsonrpc_error are used by both transports.
        // Verify they produce spec-compliant JSON-RPC 2.0 regardless.
        let success = jsonrpc_success(&json!(42), json!({"data": "ok"}));
        assert_eq!(success["jsonrpc"], "2.0");
        assert_eq!(success["id"], 42);
        assert_eq!(success["result"]["data"], "ok");
        assert!(success.get("error").is_none());

        let error = jsonrpc_error(Some(&json!("req-1")), -32601, "not found");
