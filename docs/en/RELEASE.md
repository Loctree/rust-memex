...
`rust-memex` is ready to release when the code path, distribution path, and
...
- the bundle generator emits `rust-memex-sha256sums.txt` next to the tarballs
...
curl -LO https://github.com/Loctree/rust-memex/releases/latest/download/rust-memex-x86_64-unknown-linux-gnu.tar.gz
curl -LO https://github.com/Loctree/rust-memex/releases/latest/download/rust-memex-sha256sums.txt
grep rust-memex-x86_64-unknown-linux-gnu.tar.gz rust-memex-sha256sums.txt
```

Then confirm:

- `rust-memex serve` starts cleanly
- installer-based installs expose `rust_memex` as a legacy compatibility symlink
